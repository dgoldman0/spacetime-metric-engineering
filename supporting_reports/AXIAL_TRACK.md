...
with data in [data/axial_track_gate](data/axial_track_gate/). Wall probes:
`scripts/run_axial_wall_probes.py` with data in
[data/axial_wall_probes](data/axial_wall_probes/).

## Result

The service track embeds exactly in one flat space as a tube around an axis.
Inside the tube core, spacetime is the product of the service metric with a
flat transverse plane. The along-track null energies vanish identically
there, the stress is Type I, and both transverse pressures equal \(-K/8\pi\),
where \(K\) is the Gaussian curvature of the service metric. Beyond the tube
wall and beyond the service cutoff the metric is exactly Minkowski: the
largest stress component outside the wall is 0 at every sample. The end
transitions and their opening requirement are gone.

The tube wall matches the service fields to that exterior, and with the
transplanted beta075 service it fails the geometry-demand gate. In the base
design, 74,700 of the 374,187 non-vacuum wall points are Type IV (20%). They
occur at 2,844 of the 21,384 \((\sigma,z)\) samples, including all 217 live
packet samples. Halving the jet step gives 74,702, so the layer is stable
under refinement. Field attribution on the same data gives no Type IV for the
lapse alone, 120,363 points for the carry shift alone and 53,576 for the
stretch alone.
...
static part changes sign. Third, a lapse carries no energy flux, so a wall
with a static spatial metric and zero shift is Type I for any lapse history.

The probes locate a wall structure that passes. The static stretch transition
comes first, the shift transition follows where the stretch has returned to
one, and a strong lapse gradient (\(\log\alpha\gtrsim4\)) envelops both. A
stretch that decays inside this envelope leaves a band narrowed by the large
lapse: its width is \(1.3\times10^{-5}\) at a decay rate of 0.3. The
transplanted service meets neither condition. Its decompression and packet
radial windows vary the stretch in time, and its carry shift sits partly at
the support edges, where the service lapse is weak.
...
In the core \(r\le r_c\), the fields \(\alpha\), \(A=\sqrt{\gamma_{\ell\ell}}\)
and \(\beta\) are the C∞ service fields of the constant-radius track, with the
rail coordinate read as \(z\) and the same service cutoff over
\(4\le|z|\le5\). Across the wall \(r_c\le r\le r_c+w\), the blend
\(\chi(r)=1-S(u)\) takes the flattened minimum-jerk step \(S\) of the
constant-radius design rule, with join fraction 0.1. Its argument is either
linear, \(u=(r-r_c)/w\), or logarithmic, \(u=\log(r/r_c)/\log(1+w/r_c)\).
The blend applies to \(\log\alpha\), \(\log A\) and \(\beta\). The transverse
profile is flat, \(C=r\). Alternatively it is a string core,
\(C''=-K_\Sigma\chi(r)C\), a spherical cap in the core joined to a cone
outside.

| Design | Core radius | Wall width | Spacing | Transverse profile |
|---|---:|---:|---|---|
| base | 1.75 | 1 | linear | flat |
| wall 0.5, 2, 4 | 1.75 | 0.5, 2, 4 | linear | flat |
| wall 4 log, 16 log | 1.75 | 4, 16 | logarithmic | flat |
| core 1, core 3 | 1, 3 | 1 | linear | flat |
| string core | 1.75 | 1 | linear | \(K_\Sigma=1/1.75^2\) |
...
Each sample evaluates the core and 96 composite Gauss–Legendre wall radii,
split at the flattened joins, plus one exterior radius.
...
\(\beta\) to zero, and a wall part, in which every term carries such a
derivative. The derivation verifies exactly that the product \((n,z)\) block
equals \((C''/C)\,\eta\), that the product \(rr\) and \(\phi\phi\)
components coincide, and that \(e_\phi\) is an eigenvector everywhere.

| Check | Result |
|---|---|
| Generated tensor against a brute-force four-dimensional finite-difference kernel, analytic fields at 8 wall points | largest relative difference \(2.2\times10^{-5}\) |
| Same comparison on the service metric, worst of three wall points | differences fall as the square of the brute-force step: \(6.7\times10^{-4}\), \(1.7\times10^{-4}\), \(4.1\times10^{-5}\) for steps 0.005, 0.0025, 0.00125 |
| Core transverse pressure against the spherical track's \(p_\Omega\) at six service points | largest absolute difference \(2.9\times10^{-4}\) |
| Flat fields | exact vacuum at all radii |
| Static string core | \(\rho=-p_z=K_\Sigma\chi/8\pi\) exactly; the deficit angle equals \(2\pi\int KC\,dr\) to \(10^{-9}\) |
...
The core is an exact product, so its stress is
\(T=\mathrm{diag}(K_\Sigma,-K_\Sigma,-K,-K)/8\pi\). Its minimum null energy
is \(\min(0,(K_\Sigma-K)/8\pi)\). On the current schedule, the flat core
reaches −0.283 at (1.6, 2.1). That is the transverse counterpart of the
spherical track's angular minimum −0.270; the difference is the missing
\(1/(8\pi R^2)\) cushion. The slow schedules raise the core minimum to
−0.065, and the half- and quarter-rate columns of the reset comparison carry
over directly.

A string core with \(K_\Sigma=1/1.75^2\) restores the spherical cushion. The
cost is a deficit angle of 4.53 rad (72% of \(2\pi\)) and a line density of
0.18 in geometric units. The cone also extends along the whole axis. Because
the worst core points have \(K\gg K_\Sigma\), the minimum moves only from
−0.283 to −0.270.
...
| Case | Type IV / non-vacuum wall points | Samples with Type IV | Wall minimum null | Static wall minimum | Static radial ANEC | Wall content | Core content |
|---|---:|---:|---:|---:|---:|---:|---:|
| base | 74,700 / 374,187 | 2,844 | −2.18 | −1.64 | −0.308 | 128.7 | 4.23 |
| base, half jet step | 74,702 / 374,187 | 2,846 | −2.18 | −1.64 | −0.308 | 128.7 | 4.23 |
| wall 0.5 | 71,934 / 378,251 | 2,735 | −8.27 | −6.40 | −0.603 | 200.2 | 4.23 |
| wall 2 | 78,032 / 370,122 | 2,964 | −0.85 | −0.42 | −0.159 | 101.5 | 4.23 |
| wall 4 | 76,098 / 366,700 | 2,987 | −0.42 | −0.11 | −0.082 | 107.1 | 4.23 |
| wall 4 log | 71,734 / 364,412 | 2,936 | −0.40 | −0.24 | −0.073 | 97.6 | 4.23 |
| wall 16 log | 59,255 / 359,397 | 2,896 | −0.31 | −0.052 | −0.022 | 208.7 | 4.23 |
| core 1 | 74,948 / 374,181 | 2,881 | −2.20 | −1.68 | −0.315 | 89.0 | 1.38 |
| core 3 | 74,426 / 374,193 | 2,832 | −2.16 | −1.61 | −0.303 | 195.0 | 12.44 |
| string core | 67,578 / 1,914,977 | 2,596 | −2.14 | −1.58 | −0.299 | 90.8 | 3.30 |
...
Wall width redistributes the cost. It leaves the Type IV layer in place. The
...
plane's logarithmic capacity. The static wall violates the null energy
condition over 99.5% of its area.

![Wall minimum null energy](data/axial_track_gate/wall_min_null_map_base.png)

Slower resets thin the band at each sample and hold the stretch longer, so
more samples carry it. The Type IV share of non-vacuum wall points falls from
20% to 7%, while the affected samples rise from 2,844 to 7,871.

## Why the wall has Type IV

![Type IV attribution](data/axial_track_gate/wall_type_iv_attribution.png)

The attribution cases keep one field in the core and set the others flat. The
shift-driven Type IV follows the carried packet to \(\sigma\approx4\). The
stretch-driven bands cover the whole support during decompression and
concentrate near \(\sigma\approx1.8\), where the logarithmic decay rate of
the stretch is largest. The lapse alone produces 348,450 Type I points and no
Type IV. Three identities, each checked exactly in
`tests/test_axial_track.py`, give the mechanism.

**Shear identity.** In a wall with \(\alpha=A=1\) and any
...
condition holds wherever a shift begins to vary and across the whole wall
for a weak shift. A static shift window of amplitude 0.6 is Type IV at 6,933
of 7,942 wall radii. The spherical track carries its shift uniformly over
...
track. In a static wall the two Hessians coincide. Time dependence splits
...
lapse wall is Type I at every probe radius.

## What a one-space wall requires

The staged probes give the lapse, stretch and shift separate walls. Each
field has its own start radius and width, and the lapse envelope spans
1.75–5.75.

| Staged wall | Type IV / non-vacuum radii | Minimum null |
|---|---:|---:|
...
terms in \(\rho+p_z\). The one-space wall therefore needs three properties:
...
3. all time dependence of the stretch confined to the \(r\)-independent core.

Continuity joins the core stretch to the wall's static stretch, so condition
3 holds only for a stretch that stays static. A one-space service therefore
keeps its stretch as a standing structure and runs its clocks, carry and
packet windows through the lapse and the shift. The locally deepest staged
wall reaches −6.30. Its widths and envelope strength set that level, and
they remain free.

## Comparison with the end transitions

| Measure | Two end transitions (spherical track) | Axial wall, base | Axial wall, logarithmic 16 |
|---|---:|---:|---:|
...
| Hawking–Ellis type in service | Type I everywhere | Type IV at 20% of wall points | Type IV at 16% of wall points |

The end transitions are static and ultrastatic, so their cost stays fixed
through the service. The axial wall surrounds the whole stretched support
...
exchange its tube wall carries a refinement-stable Type IV layer, set by the
carry shift and the decompressing stretch, along with 12 times the static
negative-null content. A passing one-space track requires a service revision
with a standing stretch, lapse envelopes around every shear layer, and a
reset that acts through the lapse and the shift.
