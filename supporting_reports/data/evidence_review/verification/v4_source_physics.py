...
def sec_fphi():
    ...
    def sech2(x):
        """sech^2 without overflow: 4 e^{-2|x|}/(1 + e^{-2|x|})^2."""
        e = np.exp(-2.0 * np.abs(x))
        return 4.0 * e / (1.0 + e)**2

    profiles = {
        "Poschl-Teller V0=0.5 (1 bound state)": (lambda x: -0.5 * sech2(x), 16.0),
        "Poschl-Teller V0=3.75 (2 bound states)": (lambda x: -3.75 * sech2(x), 16.0),
        "Poschl-Teller V0=12 (3 bound states)": (lambda x: -12.0 * sech2(x), 16.0),
...
def sec_anomaly():
    ...
    # Weyl law and first heat coefficient on the unit S^3 (R = 6, Vol = 2 pi^2, (4 pi)^{-3/2} Vol = sqrt(pi)/4):
    #   conformal Laplacian: a1 = R/6 - R/6 = 0 ; D^2 on 4-spinors: a1 = tr(R/6 - R/4) = 4(1 - 3/2) = -2
    w3 = sp.sqrt(sp.pi) / 4
    record(S, "S^3 conformal scalar trace = (sqrt(pi)/4) tau^-3/2 exactly (Weyl term, a1 = 0)",
           zero(laurent_coeff(s3["scalar"], s, -3) - w3) and zero(laurent_coeff(s3["scalar"], s, -1)))
    record(S, "S^3 Dirac (4-spinor) trace = 4 w tau^-3/2 - 2 w tau^-1/2, w = sqrt(pi)/4 (Weyl term, Lichnerowicz a1)",
           zero(laurent_coeff(s3["Dirac"], s, -3) - 4 * w3) and zero(laurent_coeff(s3["Dirac"], s, -1) + 2 * w3))
