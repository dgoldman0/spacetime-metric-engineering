...
- Script: `v4_source_physics.py` (sympy, mpmath, numpy/scipy; written from scratch, nothing imported
  or copied from the repository).
- Run logs: `v4_source_physics_run_final.log` and `v4_source_physics_run_final_repeat.log`.
- Run: `nice -n 10 env OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 python3 v4_source_physics.py`
  (one process, about 210 s; individual sections can be selected, e.g. `... v4_source_physics.py qi casimir`).
- Outcome: **192 of 192 checks pass.** The two final runs are identical apart from the timing lines.
  Check labels quoted below appear verbatim in the logs.
- Provenance of the earlier state: `v4_source_physics_run.log` (190 checks) was produced by the version
  saved as `v4_dev/v4_source_physics_snapshot_for_run_log1.py`. The final version adds two S³ spectrum
  checks and an overflow-free sech². The first draft of this record is `v4_dev/v4_source_physics_md_draft1.md`.
...
The bound L ≤ 52 μm/√Q adds a physical premise: the strong-coupling length must lie below the shortest
length at which Newtonian gravity has been tested. The 52 μm figure (Lee et al. 2020) is not in the local
texts.
...
1. The mirror is a cold, degenerate gas of unit-charge carriers (electrons).
   - For carriers of charge Ze, the ultra-relativistic ratio before neutralization scales as Z⁻⁴.
   - Heavier carriers raise the non-relativistic ratio as (M/m_e)².
   - At L = 1 m the gap is 2.3×10⁻¹⁸ m, about 400 times below the femtometre scale of nuclei.
     The required carrier density there is n = k_F³/3π² ≈ 4×10⁵⁷ m⁻³, from k_F = 112.9/a. At such
     densities the electron gas is the natural idealization, and the Z⁻⁴ escape route has no material
     realization.
...
2. **Independent spectral (ζ-function) determination of (a, c, d) for all three species.** d is a
   ...
   - The small-t expansions were done by exact Euler–Maclaurin series (midpoint form for the
     half-integer S³ Dirac sum) and checked against direct sums to 10⁻¹⁵ relative error or better.
   - Every spectrum was also checked against Weyl's law and the first heat coefficient (R/6 − E for
     ...
     - S²: scalar 1/τ + 1/3, Dirac 2/τ − 1/3, 1-forms 2/τ − 4/3;
     - S³: conformal scalar (√π/4)τ^{−3/2} with no τ^{−1/2} term, Dirac 4-spinor
       √π τ^{−3/2} − (√π/2)τ^{−1/2}, 1-forms (3√π/4)(τ^{−3/2} − τ^{−1/2}).
...
A fourth point is structural: the general boost-invariant homogeneous tensor with this trace is a
particular solution plus x(−1, 1, −1), and a₀ absorbs x.
...
**Derivation sketch.** Write E_n = −(m/2πn)K₁(z) with z = 2nmd. Since dK₁/dz = −K₀ − K₁/z, the force is
F_n = −∂E_n/∂d = −(m²/π)(K₀ + K₁/z), so d·|F_n| = |E_n|(1 + zK₀/K₁). DEC gives the support's energy
≥ ∫|p| dV = |F|d.
