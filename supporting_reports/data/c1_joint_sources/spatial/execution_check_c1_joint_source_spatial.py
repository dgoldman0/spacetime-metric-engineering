...
def main():
    ...
    if not args.assess_only:
        if output.exists() and any(output.iterdir()):p.error('empty spatial directory required for computation')
        output.mkdir(parents=True,exist_ok=True)
        inherited=next(r for r in parent['cases'] if r['source']=='broad_left')
        tasks=[]
        for x in control['coordinates']:
            for mass,cut in control['mass_cut_pairs']:
                case=dict(id=len(tasks),source='broad_left',coordinate=x,domain=inherited['domain'],
                ...
                tasks.append((spec,case,parent['calibration'],str(output)))
        records=[]
        ...
        (output/'tensors.json').write_text(json.dumps(dict(cases=sorted(records,key=lambda r:r['id'])),indent=2)+'\n')
        script=Path(__file__).resolve();shutil.copy2(script,output/('execution_'+script.name))
        shutil.copy2(args.spec,output/('execution_'+args.spec.name))
