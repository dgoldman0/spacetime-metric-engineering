...
def audit(spec):
    ...
    if len(options) > 2:
        raise ValueError('at most preparation and conversion-repair options are accepted')
    ...
    result = dict(label=label, factor=factor, success=True, time_samples=len(t), spatial_samples=nx,
        ...
        **conversion_metadata,
        ...
        scope='archived phase/fluid/receiver histories, explicitly recorded bounded conversion repair and initial-radiation adjustment when requested; exact registered metric energy quadrature and explicit beam replay; sampled tensor and contact checks',
...
def main():
    ...
    output.mkdir()
    specs = [(str(source), label, factor, str(output), args.prepare_radiation, args.repair_conversion_roundoff)
