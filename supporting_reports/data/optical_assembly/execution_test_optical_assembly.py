...
from adm_harness.optical_assembly import (
    ...
    splitter_matrix, store_rhs, switched_demand, retained_store_orbit,
    thermal_store_rhs, thermal_store_state,
)
...
def test_adequate_energy_capacity_does_not_prevent_store_resonance():
    ...
    assert result["retained_orbit"] is None
...
def test_smaller_store_completes_the_same_finite_trial_with_counted_energy():
    ...
    assert result["retained_orbit"]["admissible"]


def test_a_nearby_frequency_reveals_the_smaller_stores_remaining_resonance():
    result = simulate_store(reference_radius=1/(12*np.pi), half_period=.1025)
    assert not result["complete"]
    assert result["boundary"] == "tensile"
    assert 7.9 < result["time"][-1] < 8.1


def test_heat_generation_conserves_energy_and_viscous_force_enters_pressure():
    state = np.array([1.2, -.08, .27, .03])
    M, radius, k, power = 8., .03, .2, -.7
    eps = 1e-6
    gradient = np.array([(thermal_store_state(state+eps*v)["energy"]
                          -thermal_store_state(state-eps*v)["energy"])/(2*eps) for v in np.eye(4)])
    rhs = thermal_store_rhs(state, power, inventory=M, reference_radius=radius,
                            damping=k, absorption_depth=.03)
    s = thermal_store_state(state)
    assert_allclose(M*gradient@rhs, power, atol=3e-9)
    assert rhs[3] > 0
    virial = M*s["energy"]*s["speed"]**2+M*radius*state[0]*(rhs[1]-s["speed"]*power/M)
    expected = M*(s["pressure_trace"]-k*state[0]*state[1])
    assert_allclose(virial, expected, atol=1e-14)


def test_thermalized_absorption_preserves_energy_but_reduces_useful_charge():
    x, radius, alpha = 1/np.sqrt(.6), 1/(12*np.pi), .04
    z = .5*(x*x-1)
    solution = solve_ivp(lambda t, y: thermal_store_rhs(y, 0., reference_radius=radius,
        absorption_depth=alpha), (0, 3), [x, 0, z, 0], method="DOP853", rtol=1e-10, atol=1e-12)
    state = solution.y
    expected = z*np.exp(-alpha*solution.t/(2*np.pi*radius*x))
    assert_allclose(state[2], expected, atol=2e-10)
    assert_allclose(state[2]+state[3], z, atol=1e-12)
    assert_allclose(thermal_store_state(state)["energy"], x, atol=2e-11)
    assert np.all(np.diff(state[3]) >= -1e-14)


def test_contained_damping_can_trade_mechanical_failure_for_useful_charge_exhaustion():
    short = simulate_store(damping=.2)
    extended = simulate_store(damping=.2, intervals=100)
    assert short["complete"]
    assert short["thermal_energy"][-1] > .5
    assert extended["boundary"] == "extraction"
    assert 37 < extended["time"][-1] < 38
    assert extended["state"][0].min() > 1.17
    assert extended["energy"].min() > extended["reference_inventory"]
    assert extended["thermal_energy"][-1] > short["thermal_energy"][-1]
    assert_allclose(extended["extraction_depth"][-1], 1., atol=2e-12)
    assert np.max(abs(extended["balance_error"])) < 2e-8


def test_small_damped_store_contains_heat_and_passes_the_nearby_frequency_trial():
    result = simulate_store(damping=.2, reference_radius=1/(12*np.pi), half_period=.1025)
    assert result["complete"]
    assert result["thermal_energy"][-1] > .01
    assert result["hoop_stress_energy_ratio"].max() < 1
    assert np.min(np.diff(result["entropy_proxy"])) >= -1e-12
    assert np.max(abs(result["balance_error"])) < 2e-8


def test_post_trial_orbit_bounds_continued_undamped_motion():
    state = np.array([1.24, .07, .29, 0.])
    bound = retained_store_orbit(state)
    solution = solve_ivp(lambda t, y: thermal_store_rhs(y, 0.), (0, 8), state,
                         rtol=1e-10, atol=1e-12, max_step=.01)
    assert solution.y[0].min() >= bound["radius_minimum"]-1e-9
    assert solution.y[0].max() <= bound["radius_maximum"]+1e-9
