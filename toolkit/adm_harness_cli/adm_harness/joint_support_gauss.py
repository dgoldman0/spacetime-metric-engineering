"""Joint support inverse with two temporal momentum collocation points per panel.
...
def build_spacetime(t, x, coefficients, number, thermal, *, fraction=1., subpanels=1,
    ...
    for i in range(nf):
        lo, hi = i//2, i//2+1
        weight = temporal_nodes[i%2]
        value = ((lo,1-weight),(hi,weight))
        dt = t[hi]-t[lo]
        for j in range(nx-1):
            items = []
