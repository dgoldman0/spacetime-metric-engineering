...
def recompute_adm_fields(fields: dict[str, np.ndarray], recompute_r3: bool = False) -> dict[str, np.ndarray]:
    ...
    The metric convention is
        ds^2 = -alpha^2 dsigma^2 + A(dl + carrying_flow dsigma)^2 + B dOmega^2
    where carrying_flow is the exact-builder internal radial shift array.
    ...
    carrying_flow_l = np.gradient(carrying_flow, l, axis=1, edge_order=2)

    K_ll = (-A_s + 2.0 * A * carrying_flow_l + A_l * carrying_flow) / (2.0 * alpha)
    K_oo = (-B_s + carrying_flow * B_l) / (2.0 * alpha)
    k_l = K_ll / A
    k_omega = K_oo / B
    K = k_l + 2.0 * k_omega
    ...
    out["rho"] = rho
    out["j_l"] = j_l
    out["gtt"] = -alpha * alpha + A * carrying_flow * carrying_flow
    return out
