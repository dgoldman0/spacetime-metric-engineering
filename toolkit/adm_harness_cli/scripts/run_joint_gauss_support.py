...
from adm_harness.joint_support_gauss import solve_spacetime
...
OUTPUT = BASE/'joint_gauss_support'
...
def evaluate(spec):
    ...
    result = solve_spacetime(t,x,coefficients,number,u,fraction=fraction,
                             subpanels=max(1,128//intervals),deadline=300.,angular_rounds=3)
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=4,mp_context=multiprocessing.get_context('spawn')) as pool:
        results = list(pool.map(evaluate,[(32,8,.9,'shared'),(64,4,.9,'shared'),
                                          (64,2,.9,'shared'),(64,1,.9,'shared')]))
