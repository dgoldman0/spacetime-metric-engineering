...
OUTPUT = BASE/'joint_time_projected_candidate'
...
def evaluate(spec):
    ...
    pr0,prt0,unused = bilinear(s['t'],s['x'],s['radial_volume'],t[:1],x)
    initial_rate = (prt0[0]-c['volume_rate'][0]*pr0[0])/c['D'][0]
    result = evolve_support(t,x,history.coefficients,initial,incoming,initial_rate,
                            subpanels=max(1,128//intervals))
...
def main():
    ...
    for p in (Path(__file__), previous, TARGET,
              ROOT/'toolkit/adm_harness_cli/adm_harness/time_support_projection.py',
