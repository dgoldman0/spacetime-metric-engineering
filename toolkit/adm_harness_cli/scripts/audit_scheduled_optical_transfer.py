...
def plot_evidence(output, histories):
    ...
    for label, color in (("first_n32_t4113", "#1665a5"), ("second_n32_t2057", "#bd3b35")):
        with np.load(output / (label+"_states.npz")) as a:
            ax.plot(a["x"], a["sufficient_common_loss_fraction"]*1e6, label=label, color=color)
    ax.axhline(.62, color="#555555", linestyle=":", label="Assigned 0.62 ppm per exposure")
    ax.set(xlabel="Material label", ylabel="Sufficient common loss allowance (ppm)", yscale="log")
    ax.grid(alpha=.2)
