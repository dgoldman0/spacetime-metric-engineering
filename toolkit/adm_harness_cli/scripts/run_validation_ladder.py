...
DEFAULT_PACKET_MEMBER_VARIANT = "tuned_w0569_eta200"
...
def _config_service_factor(cfg: dict[str, Any]) -> float | None:
    ...
    try:
        ...
        raise SystemExit(f"Invalid service factor V in config: {v_value!r}") from exc


def _service_factor_label(v: float) -> str:
    value = float(v)
    if value.is_integer():
...
def _service_factor_case_token(v: float) -> str:
    value = float(v)
    if value.is_integer():
        return f"V{int(value)}"
    return f"V{_token(value)}"
...
def _v_label(cfg: dict[str, Any]) -> str:
    v = _config_service_factor(cfg)
    if v is None:
        return "v"
    return _service_factor_label(v)


def _infer_service_factor(
    args: argparse.Namespace,
    baseline_cfg: dict[str, Any],
    target_cfg: dict[str, Any],
) -> float:
    candidates: list[tuple[str, float]] = []
    if getattr(args, "service_factor", None) is not None:
        candidates.append(("argument", float(args.service_factor)))
    for label, cfg in [("baseline config", baseline_cfg), ("target config", target_cfg)]:
        value = _config_service_factor(cfg)
        if value is not None:
            candidates.append((label, value))
    if not candidates:
        raise SystemExit("Cannot infer service factor V; pass --service-factor or set service.velocity in both configs.")

    service_factor = candidates[0][1]
    mismatches = [(label, value) for label, value in candidates if not np.isclose(value, service_factor)]
    if mismatches:
        detail = ", ".join(f"{label}={value:g}" for label, value in candidates)
        raise SystemExit(f"Service factor V mismatch across ladder inputs: {detail}")
    return service_factor


def _default_packet_member(service_factor: float) -> str:
    return f"highres_41x73/{_service_factor_case_token(service_factor)}_{DEFAULT_PACKET_MEMBER_VARIANT}.csv"


def _resolve_output_root(args: argparse.Namespace, v_label: str) -> tuple[Path, bool]:
    if args.output_root:
        return Path(args.output_root), False
    return Path("runs") / f"{v_label}_validation_ladder", True


def _resolve_packet_member(args: argparse.Namespace, service_factor: float) -> tuple[str, bool]:
    if args.packet_member:
        return str(args.packet_member), False
    return _default_packet_member(service_factor), True
...
def _series_extreme_or_none(series: pd.Series, op: str) -> float | None:
    if series.empty:
        return None
    values = series.astype(float)
    if op == "min":
        return _float_or_none(values.min())
    if op == "max":
        return _float_or_none(values.max())
    raise ValueError(op)


def _ramp_envelope(ramp_summary: pd.DataFrame) -> dict[str, Any]:
    if ramp_summary.empty:
        return {
            "enabled": False,
            "last_no_warning_pass_amplitude": None,
            "last_hard_pass_amplitude": None,
            "first_warning_amplitude": None,
            "first_hard_failure_amplitude": None,
            "status_counts": {},
        }

    status_counts = (
        ramp_summary["status_class"].fillna("unknown").astype(str).value_counts().sort_index().to_dict()
        if "status_class" in ramp_summary.columns
        else {}
    )
    hard_pass = (
        ramp_summary[ramp_summary["hard_gate_passed"].astype(bool)]
        if "hard_gate_passed" in ramp_summary.columns
        else pd.DataFrame()
    )
    clean_pass = (
        ramp_summary[ramp_summary["status_class"].eq("pass")]
        if "status_class" in ramp_summary.columns
        else pd.DataFrame()
    )
    warnings = (
        ramp_summary[ramp_summary["warning_flag"].astype(bool)]
        if "warning_flag" in ramp_summary.columns
        else pd.DataFrame()
    )
    hard_failures = (
        ramp_summary[~ramp_summary["hard_gate_passed"].astype(bool)]
    ...
    return {
        "enabled": True,
        "last_no_warning_pass_amplitude": _series_extreme_or_none(clean_pass["amplitude"], "max"),
        "last_hard_pass_amplitude": _series_extreme_or_none(hard_pass["amplitude"], "max"),
        "first_warning_amplitude": _series_extreme_or_none(warnings["amplitude"], "min"),
        "first_hard_failure_amplitude": _series_extreme_or_none(hard_failures["amplitude"], "min"),
...
def _config_provenance(config_path: Path) -> dict[str, Any]:
    cfg = load_config(config_path)
    service = cfg.get("service", {}) or {}
    return {
        "path": str(config_path),
        "run_name": cfg.get("run_name"),
        "service_factor": _config_service_factor(cfg),
        "inputs": cfg.get("inputs", {}),
        "substrate": cfg.get("substrate", {}),
        "thresholds": cfg.get("thresholds", {}),
        "carrying_flow": service.get("carrying_flow", {}),
    }
...
def run_ladder(args: argparse.Namespace) -> Path:
    ...
    baseline_run = str(baseline_cfg_input["run_name"])
    ...
    required = decision[decision["required"].astype(bool)]
    ramp_envelope = _ramp_envelope(ramp_summary)
    metadata = {
        "schema_version": 2,
        "git_commit": _git_commit(),
        "git_dirty": _git_dirty(),
        "service_factor": service_factor,
        "service_factor_label": v_label,
        ...
        "result_classes": {
            "target_required_gates": {
                "passed": bool(required["passed"].astype(bool).all()),
                "decision_sheet": "validation_ladder_decision_sheet.csv",
            },
            "amplitude_ramp_envelope": ramp_envelope,
        },
        "config_provenance": {
            "baseline": _config_provenance(baseline_config),
            "input_target": _config_provenance(target_config_input),
            "target": _config_provenance(target_config),
            "negative_counterpart": _config_provenance(negative_config),
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional active-rail service factor V. If omitted, it is inferred from service.velocity in the configs.",
    )
    ...
    parser.add_argument(
        "--packet-member",
        default=None,
        help="Member CSV inside --packet-input. Defaults to highres_41x73/V<service-factor>_tuned_w0569_eta200.csv.",
    )
...
def main() -> int:
    ...
    print(f"Wrote {metadata.get('service_factor_label', metadata.get('v_label', 'v')).upper()} validation ladder: {root}")
