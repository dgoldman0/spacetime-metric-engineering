#!/usr/bin/env python3
"""Refine support with the incoming pressure history at the right material cut."""
...
OUTPUT = BASE/'joint_projected_candidate'
...
def evaluate(spec):
    intervals, time_factor = spec
    start = time.monotonic(); history = ProjectionHistory(); h = history.h
    s = history.target
    native = h.reference.t
    t = native if time_factor == 1 else np.unique(np.r_[native,(native[:-1]+native[1:])/2])
    ...
    # Reverse radial orientation. The physical pressure is unchanged; the
    # spatial projections of velocity, acceleration, force, and grad ln R
    # change sign. v<0 makes the right cut the incoming pressure boundary.
    def reversed_coefficients(at, ay):
        cc = history.coefficients(at, -ay)
        for key in ('v', 'acceleration', 'angular_gradient', 'fixed_force'):
            cc[key] = -cc[key]
        return cc
    result = project_balance(t, -x[::-1], reversed_coefficients, initial[::-1], incoming,
                             spatial_subpanels=max(1, 128//intervals))
    for key in ('support_energy', 'radial_pressure', 'local_exchange'):
    ...
    label = f'n{intervals}_timefactor{time_factor}'
    summary = dict(label=label, intervals=intervals, time_refinement_factor=time_factor, time_nodes=len(t),
        **audit, **{k:v for k,v in result.items() if not isinstance(v, np.ndarray)},
        ...
        member_energy_admissible=audit['maximum_separate_member_density_shortfall']<1e-7 and audit['minimum_support_density']>=0,
...
def main():
    ...
    for p in (Path(__file__), previous, TARGET,
              ...
              BASE/'joint_support_smooth_allocation/manifest.json',
    ...
    with ProcessPoolExecutor(max_workers=min(4, args.workers), mp_context=multiprocessing.get_context('spawn')) as pool:
        results = list(pool.map(evaluate, [(128,1), (256,1), (128,2), (256,2)]))
