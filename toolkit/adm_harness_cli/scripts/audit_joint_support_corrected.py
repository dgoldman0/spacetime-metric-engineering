#!/usr/bin/env python3
"""Corrected continuum audit, retaining temporal field momentum under a boost.

The archived audit_joint_support.py is preserved as the producer of the
previous evidence. This revision supersedes its force residual diagnostics.
"""
...
OUTPUT=BASE/'joint_support_audit_corrected'
...
def continuum_check(h,state,*,preserve_reference_fluid=False):
    ...
    # The material-frame spatial projection mixes normal-frame force and
    # power. A changing electric field contributes -v times its rest power.
    field_force=-(field_x-share_x)/(ell*radius**4)-c['v']*charge
    ...
    return dict(pointwise_force_max=float(abs(force).max()),pointwise_power_max=float(abs(power).max()),
        ...
        temporal_field_momentum_retained=True,
...
def main():
    ...
    for path in (Path(__file__),ROOT/'toolkit/adm_harness_cli/tests/test_joint_support_audit_corrected.py',
                 ROOT/'supporting_reports/data/joint_support_audit/manifest.json'):
