...
from adm_harness.beta075_service_aligned_schedule import (  # noqa: E402
    ServiceAlignedScheduleSpec,
    build_service_aligned_schedule,
...
def _parse_widths(value: str) -> tuple[int, ...]:
    ...
    if not widths:
        raise argparse.ArgumentTypeError("at least one width step is required")
    if any(width <= 0 for width in widths):
        raise argparse.ArgumentTypeError("width steps must be positive")
    return widths


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run service-time-aligned scheduled evolution for the sealed beta075 package."
    )
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("toolkit/adm_harness_cli/runs/stage2_beta075_service_aligned_schedule"),
    )
    ...
    parser.add_argument("--width-steps", type=_parse_widths, default=(1, 3, 5, 7, 9, 15, 31, 47))
...
def main() -> int:
    ...
    spec = ServiceAlignedScheduleSpec(
        ...
        width_steps=tuple(args.width_steps),
    ...
    outputs, metadata = build_service_aligned_schedule(
        args.closure_dir,
        args.source_coupling_dir,
        symbol_spec=PrincipalSymbolSpec(),
        spec=spec,
    )
    files = write_service_aligned_schedule_outputs(args.outdir, outputs, metadata)
    print(json.dumps({
        "ok": True,
