...
OUTPUT = BASE/'joint_support_residual'
...
def evaluate(spec):
    intervals,stride,fraction,allocation,cap = spec
    ...
    result = solve_residual(t,x,coefficients,number,u,fraction=fraction,material_cap=cap,
...
def main():
    ...
    for p in (Path(__file__),previous,ROOT/'toolkit/adm_harness_cli/adm_harness/joint_support_gauss.py',
              ROOT/'toolkit/adm_harness_cli/adm_harness/joint_support_residual.py',
    ...
    with ProcessPoolExecutor(max_workers=4,mp_context=multiprocessing.get_context('spawn')) as pool:
        results = list(pool.map(evaluate,[(32,8,.9,'shared',.2),(64,4,.9,'shared',.2),
                                          (64,1,.9,'shared',.2),(64,1,.9,'shared',.4)]))
