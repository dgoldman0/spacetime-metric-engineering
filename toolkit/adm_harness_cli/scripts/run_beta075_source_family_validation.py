...
from adm_harness.beta075_source_family_validation import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the beta075 formal source-family equations against derived support-reservoir principal symbols."
    )
    parser.add_argument(
        "--equation-dir",
        type=Path,
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("toolkit/adm_harness_cli/runs/stage2_beta075_source_family_validation"),
    )
    return parser
...
def main() -> int:
    ...
    inputs = SourceFamilyValidationInputs(
        ...
        runs=default_validation_runs(),
    )
    outputs, metadata = build_source_family_validation(inputs)
    files = write_source_family_validation_outputs(args.outdir, outputs, metadata)
    print(json.dumps({
        "ok": True,
