...
OUTPUT = BASE/'joint_support_least_squares'
...
def evaluate(spec):
    ...
    result = solve_least_squares(t,x,coefficients,number,u,reference,fraction=fraction,material_cap=cap,
                                subpanels=max(1,128//intervals),deadline=180.,
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=4,mp_context=multiprocessing.get_context('spawn')) as pool:
        results = list(pool.map(evaluate,[(32,4,.9,'linear_basis',.2,None),(64,2,.9,'linear_basis',.2,None),
                                          (64,1,.9,'linear_basis',.2,None),(32,4,.9,'linear_basis',.4,.05)]))
