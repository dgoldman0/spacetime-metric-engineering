...
from adm_harness.beta075_aligned_envelope_certificate import (  # noqa: E402
    AlignedEnvelopeSpec,
    build_aligned_envelope_certificate,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the beta075 service-aligned convex-kernel envelope certificate."
    )
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("toolkit/adm_harness_cli/runs/stage2_beta075_aligned_envelope_certificate"),
    )
...
def main() -> int:
    ...
    outputs, metadata = build_aligned_envelope_certificate(
        args.closure_dir,
        args.source_coupling_dir,
        symbol_spec=PrincipalSymbolSpec(),
        spec=spec,
    )
    files = write_aligned_envelope_outputs(args.outdir, outputs, metadata)
    print(json.dumps({
        "ok": True,
