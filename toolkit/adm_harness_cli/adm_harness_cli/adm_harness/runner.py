...
from .absorbers import compute_field_delta
...
def run_from_config(config_path: str | Path, output_dir: str | Path | None = None) -> Path:
    ...
    if synthesis_enabled:
        ...
        delta_result = compute_field_delta(
            exact_fields,
            preliminary_ledger=preliminary_ledger,
            substrate_fields=substrate_fields,
            absorber_cfg=cfg.get("absorber", {}),
    ...
    if synthesis_enabled and cfg.get("absorber", {}).get("target_field", "beta") != "beta":
        status["caveats"].append("Non-beta synthesis uses an experimental numerical R3 recomputation path.")
