...
def validate_config_contract(cfg: dict[str, Any]) -> ValidationReport:
    ...
    if synthesis.get("enabled", False):
        target = absorber.get("target_field", "beta")
        if target not in ALLOWED_DELTA_KEYS:
            report.errors.append(f"absorber.target_field must be one of {sorted(ALLOWED_DELTA_KEYS)}; got {target!r}")
        law = absorber.get("law") or absorber.get("mode")
        if not law:
            report.errors.append("synthesis enabled but absorber.law/mode is missing")
    return report
