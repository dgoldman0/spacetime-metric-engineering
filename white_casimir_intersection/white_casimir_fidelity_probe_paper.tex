...
central measurement path participates. Here ``readout'' is shorthand for the
proposed detection ensemble: path, observable, controls, and ordinary
apparatus backgrounds.
...
transit path within the readout ensemble, and the exterior region as far-field
control.
...
and readout radius. This creates useful separability and a demanding physical gate:
...
same source-shell placement and keeps the central measurement channel small.
...
is assigned to the near-shell band. The explicit central-transit mask receives
...
Stage 4 close the direct-readout route for the modeled apparatus: the
shell-shaped morphology remains real in the proxy, and the corresponding
physical readout amplitude sits below the level where the schedule can recover
it. The perturbation schedule is therefore a diagnostic for any proposed
transduction mechanism. A viable experimental claim must first show a concrete
propagation or readout channel that produces a shell-coded observable at
...
The scale and readout layers are the gate for the White-centered detection
ensemble: voltage and current response in the custom cavity and the central
current, photon, or electron transit comparison in the sphere-cylinder toy
model.
The calibrated proxy places the gravitational/source magnitude many orders
beneath representative Alcubierre source demand and gives a linearized timing
bound of order $10^{-76}\,\mathrm{s}$. Stage 4 then asks whether
geometry-coded perturbations could recover a shell template from ordinary
electromagnetic, material, drift, vibration, and readout backgrounds. Under
the block-bootstrap, geometry-derived nuisance model, recovery becomes
reliable near signal-to-background ratio $1.2$, with an ordinary-EM-only
false-positive rate near $0.034$. That threshold is demanding because the
calibrated Casimir/metric signal assigned to the central measurement path sits
far below the electromagnetic/material background scales. The combined Stage
3/Stage 4 result closes the direct-detection route for the modeled apparatus.

The synthetic schedule remains useful as a measurement-ensemble discriminator.
It can rank geometry perturbations and test whether a proposed readout carries
a shell-coded response with enough amplitude and template orthogonality.
Signal amplitude comes from the physical transduction model being tested. A
future positive laboratory claim must supply an independently modeled
material, propagation, or readout mechanism that lifts a shell-coded observable
to order-unity signal-to-background ratio and passes the same ordinary-EM-only
false-positive test.
