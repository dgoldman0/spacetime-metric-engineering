...
boundary-geometry question can be tested directly: does a $1\um$ sphere
centered in a $4\um$ cylinder generate a stable negative scalar morphology in
the annular region where the reported warp-like shell appears? This paper
evaluates that question with a pair-resolved, worldline-style scalar probe. The
sphere, cylinder wall, annular shell, central readout path, boundary
infrastructure, and far-field control are scored separately; scale windows,
seeds, wall thicknesses, and small geometry perturbations test whether the
organization is tied to the sphere-wall gap.

The probe resolves a shell-centered scalar morphology. The clean mid-scale
channel concentrates negative proxy magnitude in the annular source region,
places the radial maximum near the sphere-wall gap, keeps far-field leakage
small, and preserves the pattern under modest sphere/cylinder changes. The
...
this form, the White geometry is a structured scalar Casimir-morphology case.
...
comparison, and metric source demand in addition to negative scalar support.
