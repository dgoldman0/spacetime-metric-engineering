...
54. Completed light local 3+1/backreaction proxy stress with off-axis
    ...
    does not materially amplify dense V5. Read this run as a successful
    algebraic/off-axis proxy, not the final local-compute Stage II capstone.
...
Completed: light local off-axis/backreaction proxy stress. The
           partition-backed Parquet run passes hard gates across all tested
...
The first robustness ladder also holds. Expanding affine widths to `tau = 0.125, 0.25, 0.50, 1.00, 1.50, 2.00` on the `61 x 83` promoted pair scanned `120,918` windows with no benchmark-floor violations. A denser `81 x 109` compact rerun produced `8,829` point-ledger rows, zero live packet norm failures, and then scanned `105,924` affine windows with no violations. The compact `tau = 2.0` margin is stable across grids (`0.04119` at `61 x 83`, `0.04121` at `81 x 109`), so the current pass is not a coarse-grid artifact at this resolution step. The next deeper analysis should probe broader affine widths with boundary accounting and run sector-sensitivity tests, especially G angular capacity at broad widths and H/D current relaxation at short widths.

The boundary-aware adversarial pass has now run on the `81 x 109` compact target. Scoreable windows require at least `80%` two-sided affine support coverage and `80%` Gaussian kernel coverage. Scoreable broad windows at `tau = 2.0, 3.0, 4.0` still have zero benchmark-floor violations, and G/H/D/closure ablations also pass. However, the raw unfiltered `tau = 4.0` scan contains boundary-truncated violations (`189` per radial branch) that disappear only because those windows are not scoreable on the current domain. That is the next risk: the model is not killed by scoreable broad SNEC or sector ablation, but the current finite domain cannot adjudicate the broad boundary windows. The next deeper test should extend the compact source-ledger domain and rescore `tau = 3.0` and `tau = 4.0` boundary windows with adequate affine support.
...
The refined radial support-law pass has now produced a useful V5 improvement surface. Mild positive packet/support-local `gamma_ll` stretch is the useful direction; negative radial softening is not. A filled positive core stretch gives Tkk and angular-pressure peak relief, while a catch-only outer annular radial stretch recovers part of the live-fraction cost. The current balanced candidate is `core +0.05` on `entry_catch_release` plus an annular `ring +0.12` on `catch_only`.
