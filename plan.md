...
The refined radial support-law pass has now produced a useful V5 improvement surface. Mild positive packet/support-local `gamma_ll` stretch is the useful direction; negative radial softening is not. A filled positive core stretch gives Tkk and angular-pressure peak relief, while a catch-only outer annular radial stretch recovers part of the live-fraction cost. The current balanced candidate is `core +0.05` on `entry_catch_release` plus an annular `ring +0.12` on `catch_only`; it preserves zero packet-norm failures, lowers live Tkk/current/angular fractions, lowers Tkk and pOmega point peaks, and leaves only a small live `p_l` cost. The aggressive comparator is `core +0.06`, `ring +0.12`, which gives stronger peak relief with more live `p_l` cost.

The next recommended work is to refine the geometry of that radial support law, not to return to broad release/lapse sweeps. Tune ring radius and width around the balanced candidate, with the gate that live `p_l` moves closer to base while Tkk and pOmega peak relief are preserved. Use lapse later only as a small local null-channel trim after the radial geometry stabilizes.
